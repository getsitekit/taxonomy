...
return [
	...
	'uninstall' => function ($app) {
        ...
        // 卸载扩展时删除分类数据表（先删关联表，再删主表）
        $util = $app['db']->getUtility();

        if ($util->tableExists('@taxonomy_term_item')) {
            $util->dropTable('@taxonomy_term_item');
        }

        if ($util->tableExists('@taxonomy_term')) {
            $util->dropTable('@taxonomy_term');
        }
